...
plot(x1, y1, 'y.', 'DisplayName', 'Bord droit');
plot(x2, y2, 'c.', 'DisplayName', 'Bord gauche');
plot(Xgps,Ygps,'w.', 'DisplayName', 'Trajectoire non interpolé')% Tracé de la trajectoire non interpolé en bleu
plot(Xi, Yi, 'r-', 'LineWidth', 1.5, 'DisplayName', 'Trajectoire interpolée');

axis equal;
xlabel('x (m)');
ylabel('y (m)');
title('Trajectoire du véhicule sur le circuit de test');
